...
koc.prototype.writemail = function(userid, subject, message, turing){
  ...
  if( turing === undefined || turing === null || !turing.length ) {
		console.debug("[/writemail] Turing not passed, retrieving it...");
  	return _koc.requestPage(true, "GET", "writemail.php", {
  		to: userid
  	}, function (response, body) {
				console.debug("[/writemail] Got a response from writemail.php to get the turing...");
        var turing = _koc.parser.getTuring(body);
  			if( turing === undefined || turing === null || !turing.length ) {
					console.error("[/writemail] Turing not found on the page!");
					return _koc.prepareResponse({
						success: false,
						error: "An error occurred retrieving the turing"
					});
        }
			  console.debug("[/writemail] Turing found, calling itself recursively...");
        return _koc.writemail(userid, subject, message, turing);
      });
  }

	console.debug("[/writemail] Sending message...");
  return _koc.requestPage(true, "POST", "writemail.php", {
    ...
    hash: ''
		...
		console.debug("[/writemail] Response retrieved");
    ...
    if (path == "/inbox.php") {
      ...
      return _koc.prepareResponse(inbox);
		...
		console.error("[/writemail] Mail not sent properly. Current location: " + path);
    return _koc.prepareResponse({
    	success: false,
