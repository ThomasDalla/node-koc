...
koc.prototype.createRequestPromise = function( options, onSuccess ) {
    ...
    .then( function(res) {
        ...
        // If not XHR, try to parse some more information
        if( options.headers["X-Requested-With"] === undefined ) {
...
var handleRecruit = function(koc, html) {
    ...
    if(!parsedResponse.success&&parsedResponse.error=="Invalid Selection") {
        ...
        var p = koc.getReCaptchaChallenge()
        ...
        return p; // this is a promise
    }
    // add koc host and pass the response
    parsedResponse.image = koc.getKoCHost() + parsedResponse.image;
    return Q(koc.prepareResponse(parsedResponse));
};
...
koc.prototype.recruit = function(data) {
    ...
    if(data===undefined||data===null||data=={}) {
        ...
        p = _koc.requestPage("GET", "recruit.php", {}, function(response, body) {
            return handleRecruit(_koc, body);
        });
    }
    else {
        ...
        p = _koc.requestPage("POST", "recruit.php", data, function(response, body) {
            return handleRecruit(_koc, body);
        });
    }
    return p;
