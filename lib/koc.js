...
var koc = function (session) {
  ...
  this.koc_recaptcha_url = "http://www.google.com/recaptcha/api/challenge";
  this.koc_recaptcha_reload_url = "http://www.google.com/recaptcha/api/reload";
  this.koc_recaptcha_key = "6LcvaQQAAAAAACnjh5psIedbdyYzGDb0COW82ruo";
...
/**
 * ReCaptcha key for KoC
 * @return {String} recaptcha key
 */
koc.prototype.getKoCReCaptchaKey = function () {
  return this.koc_recaptcha_key;
};

/**
 * Get the reload URL to get a readable image
 * @return {String} base or the reload URL
*/
koc.prototype.getKoCReCaptchaReloadUrl = function () {
  return this.koc_recaptcha_reload_url;
};
...
koc.prototype.prepareResponse = function (result) {
  ...
  //     response.menu = this.menu;
	var successMsg = response.success === true ? "success" : "failure";
	console.log("[" + this.stats.username + "][" + response.location + "][" + successMsg + "]");
  return response;
};
 ...
 * @param {String} [referer] Manually set the referer
 * @return {Object} the options to pass to node request
 */
koc.prototype.getRequestOptions = function (method, page, params, xhr, referer) {
  var koc_session = "";
  if (this.getSession().length > 0) {
    koc_session = " " + this.getKoCSessionCookieName() + "=" + this.getSession();
  }
  if(referer==null||referer==undefined||!(referer.length)){
 ...
 * @return {Q.Promise} the HTTP request promise (use .then() and .fail() on it)
 */
 ...
 * @return {Q.Promise} the HTTP request promise (use .then() and .fail() on it)
 */
koc.prototype.requestPage = function (mustBeLoggedIn, method, page, params, onSuccess, xhr, referer) {
  ...
  if (!_koc.getSession().length) {
    ...
    return request(_koc.getRequestOptions("HEAD", ""))
      .then(function (result) {
        ...
        _koc.updateKocSession(response.headers);
        _koc.location = response.request.path;
        _koc.setres().then(function(){}).fail(function(){}); // just set res
        return _koc.createRequestPromise(_koc.getRequestOptions(method, page, params, xhr, referer), onSuccess);
      })
      .fail(function (error) {
        return _koc.prepareResponse({
          success: false,
...
koc.prototype.getReCaptchaChallenge = function () {
  ...
  return request({
    url: _koc.getKoCReCaptchaUrl(),
    qs: {
      k: _koc.getKoCReCaptchaKey(),
      cachestop: Math.random().toFixed(17),
      headers: {
        Referer: _koc.getKoCHost()
      }
    }
  })
  .then(function (result) {
    ...
    // Reload to get a readable URL
    return request({
      url: _koc.getKoCReCaptchaReloadUrl(),
      qs: {
        c: challenge,
        k: _koc.getKoCReCaptchaKey(),
        lang: "en",
    ...
    .then(function(result){
      ...
      if (response.statusCode != 200) {
        return _koc.prepareResponse({
          success: false,
          error: "Wrong response from the server reloading captcha image",
          details: "Got status " + response.statusCode
        });
      }
      var body = result[1];

      var reFinishReload = /Recaptcha\.finish_reload\('([^']+)'/;
      var mFinishReload = reFinishReload.exec(body);

      if(mFinishReload == null){
        return _koc.prepareResponse({
          success: false,
          error: "Failed to read finish_reload",
          details: {
            challenge: challenge,
            server: server,
            response: body
      ...
      var newChallenge = mFinishReload[1];

      return _koc.prepareResponse({
        success: true,
        image: _koc.getRecaptchaImage(server, newChallenge),
        server: server,
...
koc.prototype.register = function (race, username, password, email, challenge, challenge_response) {
  ...
  return this.verify(username, password, password).then(function (result) {
    if (!result.success || (!result.result.username.length && !result.result.password.length)) {
      // we need a turing
      ...
      var error = util.format("%s %s", result.result.username, result.result.password).trim();
      return _koc.prepareResponse({
        success: false,
        error: error,
        details: result
 ...
 * @param b_start {number} page (by you)
 * @param o_start {number} page (on you)
 * @returns {Q.Promise}
 */
 ...
 * @param files_start {number} page (b_start : by you)
 * @param intercepted_start {number} page (o_start : on you)
 * @returns {Q.Promise}
 */
 ...
 * @returns {Q.Promise}
 */
koc.prototype.attackfield = function() {
  var _koc = this;
 ...
 * @param asset_id {number} usually same as the user_id
 * @returns {Q.Promise}
 */
  ...
  if( turing === undefined || turing === null || !turing.length ) {
    return _koc.getQuickStats(userid)
      ...
      .fail(function (error) {
        return _koc.prepareResponse({
          success: false,
          error: "Failed to get the quick stats",
          details: error,
        });
      });
  }
    ...
    return _koc.requestPage(true, "POST", "attack.php", data, function (response, body) {
      ...
      if( response.request.uri.pathname == '/inteldetail.php' ) {
        var intelDetail = _koc.parser.parseIntelDetail(body, response.request.path, _koc.reconData.userid);
        return _koc.prepareResponse(intelDetail);
      }
      ...
      if( response.request.uri.pathname == '/attack.php' ) {
        // it can also be because the enemy is too strong for us
        // or we don't have spies!
        var errorReason = helpers.stripHtml(_koc.parser.parseErrorMessageRed(body));
        ...
        else if(_koc.reconData.attemptNb > 3) {
          return _koc.prepareResponse({
            success: false,
 ...
 * @returns {Q.Promise}
 */
koc.prototype.attackOrRaid = function(attack_type, userid, turing) {
  var _koc = this;
  if(attack_type!='attack'&&attack_type!='raid'){
    return _koc.prepareResponse({
      success: false,
      error: "Please specify the attack_type ('attack' or 'raid')",
    });
  }
  // If turing is not passed, get it from stats.php?id=userid
  if( turing === undefined || turing === null || !turing.length ) {
    return _koc.getQuickStats(userid)
      .then(function (quickStats) {
        if (quickStats.result !== undefined
          ...
          && quickStats.result.turing !== null
          && quickStats.result.turing.length) {
          return _koc.attackOrRaid(attack_type, userid, quickStats.result.turing); // recurse with the turing
        }
        return _koc.prepareResponse({
          success: false,
          error: "Failed to get the turing from the quick stats",
    ...
    var data = {
      ...
      defender_id: userid,
      attackbut: (attack_type == 'attack') ? 'Attack Now!' : 'Raid Now!',
    };
    if( _koc.attackData === undefined ) {
      // That's the first time we're called, setup our attackData
      // in case we need to recurse with a new turing
      _koc.attackData = {
        attemptNb: 0,
        userid: userid,
    ...
    _koc.attackData.attemptNb++;
    return _koc.requestPage(true, "POST", "attack.php", data, function (response, body) {
      // If we're on the intel detail, it's all good, we can parse it and return it
      if( response.request.uri.pathname == '/detail.php' ) {
        var battleReport = _koc.parser.parseBattleReport(body, response.request.path, _koc.attackData.userid);
        return _koc.prepareResponse(battleReport);
      }
      // If we have the wrong turing, we are redirected to attack.php
      // We can try to get the new turing and try again
      if( response.request.uri.pathname == '/attack.php' ) {
        ...
        else if(_koc.attackData.attemptNb > 3) {
          return _koc.prepareResponse({
            success: false,
            error: "Failed after 3 attempts",
            details: {
        ...
        var newTuring = _koc.parser.getTuring(body);
        // Call itself
        return _koc.attackOrRaid(attack_type, _koc.attackData.userid, newTuring);
      }
      return _koc.prepareResponse({
        success: false,
        error: "I don't know how to handle " + response.request.path,
 ...
 * @param userid {number} userid of the defender
 * @param turing {string=} turing of this page (optional)
 * @returns {Q.Promise}
 */
koc.prototype.attack = function(userid, turing) {
  return this.attackOrRaid('attack', userid, turing);
};
 ...
 * @returns {Q.Promise}
 */
koc.prototype.raid = function(userid, turing) {
  return this.attackOrRaid('raid', userid, turing);
};

koc.prototype.setres = function(width, height) {
  if(!width) width = 1280;
  if(!height) height = 720;
  // call setres.php
  var _koc = this;
  var p =_koc.requestPage(false, "GET", "setres.php", {
    width: width,
    height: height,
  }, function (response) {
    return _koc.prepareResponse({
...
koc.prototype.inbox = function(){
  var _koc = this;
  return _koc.requestPage(true, "GET", "inbox.php", {}, function (response, body) {
    return _koc.prepareResponse(_koc.parser.parseInbox(body));
  });
};

koc.prototype.writemail = function(userid, subject, message, turing){
  var _koc = this;

  // If turing is not passed, get it from writemail.php?to=userid
  if( turing === undefined || turing === null || !turing.length ) {
		console.log("[/writemail] Turing not passed, retrieving it...");
  	return _koc.requestPage(true, "GET", "writemail.php", {
  		to: userid
  	}, function (response, body) {
				console.log("[/writemail] Got a response from writemail.php to get the turing...");
        var turing = _koc.parser.getTuring(body);
  			if( turing === undefined || turing === null || !turing.length ) {
					console.log("[/writemail] Turing not found on the page!");
					return _koc.prepareResponse({
						success: false,
						error: "An error occurred retrieving the turing"
					});
        }
			  console.log("[/writemail] Turing found, calling itself recursively...");
        return _koc.writemail(userid, subject, message, turing);
      });
  }

	console.log("[/writemail] Sending message...");
	var referer = this.getKoCHost() + "/writemail.php?to=" + userid;
  return _koc.requestPage(true, "POST", "writemail.php", {
  	to: userid,
    subject: subject,
    message: message,
    turing: turing,
    ...
    hash: ''
		...
		console.log("[/writemail] Response retrieved");
    var path = response.request.uri.pathname;
    if (path == "/inbox.php") {
			console.log("[/writemail] We are on the inbox page, parsing it...");
    	var inbox = _koc.parser.parseInbox(body);
    	inbox.success = true;
    	inbox.message = 'Message sent!';
			console.log("[/writemail] All good, sending response...");
      return _koc.prepareResponse(inbox);
    }
		console.log("[/writemail] Mail not sent properly. Current location: " + path);
    return _koc.prepareResponse({
    	success: false,
