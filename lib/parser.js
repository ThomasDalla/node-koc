...
parser.parseInbox = function(inboxHtml) {
	...
	// Inbox
	var reMessageInfo = /<a href="stats\.php\?id=([0-9]+)">([^<]+)<\/b>\s*<\/td>\s*<td>([^<]+)<\/td>\s*<td align="right">([^<]+)<\/td>\s*<td align="left">([^<]+)<\/td>\s*<td[^>]*>\s*<form action="writemail\.php"[^>]*>\s*<input[^>]*>\s*<input[^>]*>\s*<input type="hidden" name="reply_to_message_id" value="([0-9]+)">/gmi;
	var reMessageContent = /<td style="[^"]*" bgcolor="[^"]*" colspan="4"><p style="[^"]*">([\s\S]+)<\/p><\/td>\n/gmi;
