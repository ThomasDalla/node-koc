...
parser.parseLeftSideBox = function (html) {
  ...
  var re = /<title>Kings of Chaos :: ([a-zA-Z0-9]+)'s ([^<]+)<\/title>/i;
  var m = re.exec(html);
...
parser.parseFullStats = function (html) {
  ...
  // Turing
  var turing = this.getTuring(html);
  result.turing = turing;
  // Main User Stats
  var reNameCommander = /Name:[\s<tdrbpa>\/\\]*<td>\s*([^\s]+)\s*<\/td>[\s<tdrbpa>\/\\]*Commander:\s*<\/b>\s*<\/td>\s*<td[^>]*>\s*(.*)\s*<\/td>/gmi;
  var mNameCommander = reNameCommander.exec(html);
  var reSupremeCommander = /Supreme Commander\s*<\/b>\s*<\/td>\s*<td[^>]*>\s*<a href="stats\.php\?id=([0-9]+)">([^<]+)<\/a>\s*<\/td>[\s<tdrbpa>\/\\]*Chain Name:[\s<tdrbpa>\/\\]*<td>\s*([^<]+)/gmi;
  var mSupremeCommander = reSupremeCommander.exec(html);
  var supremeCommander = mSupremeCommander === null ? {
    username: "None",
    userid: -1,
  } : {
    username: mSupremeCommander[2],
    userid: helpers.quantityFromString(mSupremeCommander[1]),
  };
  var chainName = mSupremeCommander === null ? "" : mSupremeCommander[3];
  var reUserStats = /Alliances:\s*<\/b>\s*<\/td>\s*<td>\s*(.*)\s*<\/td>[\s<tdrbpa>\/\\]*Race:[\s<tdrbpa>\/\\]*<td>\s*([^<\s]+)[\s<tdrbpa>\/\\]*Rank:[\s<tdrbpa>\/\\]*<td>([^<\s]+)[\s<tdrbpa>\/\\]*Highest Rank:[\s<tdrbpa>\/\\]*([^<]+)[\s<tdrbpa>\/\\]*Army\s*Size:[\s<tdrbpa>\/\\]*([^<\s]+)[\s<tdrbpa>\/\\]*Army Morale:[\s<tdrbpa>\/\\]*([^<\s]+)/gmi;
  ...
  var mCommander = reCommander.exec(mNameCommander[2]);
  var commanderUsername = (mCommander === null) ? mNameCommander[2] : mCommander[2];
  var commanderUserid = (mCommander === null) ? -1 : helpers.quantityFromString(mCommander[1]);
  // Parse the primary alliance
  var rePrimaryAlliance = /id=(\d*)">\s*([^<]+)\s*<\/a>\s*<\/b>\s*\(\s*Primary/gmi;
  var mPrimaryAlliance = rePrimaryAlliance.exec(mUserStats[1]);
  var primaryAlliance = mPrimaryAlliance !== null ? {
    name: mPrimaryAlliance[2],
    id: helpers.quantityFromString(mPrimaryAlliance[1]),
  } : {
    name: "",
    id: -1,
  };
  ...
  var mAlliances;
  var alliances = [];
  while ((mAlliances = reAlliances.exec(mUserStats[1])) !== null) {
    alliances.push({
      name: mAlliances[2],
      id: mAlliances[1],
  ...
  while ((mOfficers = reOfficers.exec(html)) !== null) {
    officers.push({
      ...
      userid: helpers.quantityFromString(mOfficers[1]),
      rank: mOfficers[3],
  ...
  result.user = {
    ...
    race: mUserStats[2],
    rank: mUserStats[3],
    highestRank: mUserStats[4],
    armySize: mUserStats[5],
    armyMorale: mUserStats[6],
...
/**
 * Parse the quick stats (xhr from the battlefield)
 * @param html
 * @returns {Object} parsed quick stats
 */
parser.parseQuickStats = function(html) {
  if (html === undefined || html === null || !html.length)
    return {
      success: false,
      error: "Empty page to parse",
  ...
  // Full Profile Link
  var reFullProfile = /Full Profile:<\/b><\/td>\s*<td><a href="stats\.php\?id=([0-9]+)">([^<]+)<\/a><\/td>/i;
  var mFullProfile = reFullProfile.exec(html);
  if(mFullProfile===null){
    return {
      success: false,
      error: "Failed parsing the full profile link in the quick stats",
      turing: turing,
    };
  }
  var userid = helpers.quantityFromString(mFullProfile[1], -1);
  var username = mFullProfile[2];

  // Commander
  var reCommander = /Commander:<\/b><\/td>\s*<td>([^\n]+)/i;
  var mCommander  = reCommander.exec(html);
  var commander = (mCommander!==null) ? helpers.parseUser(mCommander[1]) : { username: '???' };

  // Supreme Commander
  var reSupremeCommander = /Supreme Commander<\/b><\/td>\s*<td>([^\n]+)/i;
  var mSupremeCommander  = reSupremeCommander.exec(html);
  var supremeCommander = (mSupremeCommander!==null) ? helpers.parseUser(mSupremeCommander[1]) : { username: 'None' };


  // Highest Rank, Treasury, Fortification
  var reRankTreasuryFortification = /Highest Rank[^\n]*\s*<td>([^\n]+)<\/td>[\sa-zA-Z,-<>\/\\]+Treasury[^\n]*\s*<td>([^\n]+)<\/td>[\sa-zA-Z,-<>\/\\]+Fortifications[^\n]*\s*<td>([^<]+)/i;
  var mRankTreasuryFortification  = reRankTreasuryFortification.exec(html);
  var highestRank = '???', fortification = '???', treasuryAsText = '???', treasury = -1;
  if(mRankTreasuryFortification!==null){
    highestRank = mRankTreasuryFortification[1];
    fortification = mRankTreasuryFortification[3];
    treasury = helpers.quantityFromString(mRankTreasuryFortification[2]);
    treasuryAsText = helpers.stripHtml(mRankTreasuryFortification[2]);
  }

  // Buddy List Status
  var reBuddyStatus = /<option\s*label="([^"]+)"\s*value="([^"]+)"\s*selected="selected">/i;
  var mBuddyStatus  = reBuddyStatus.exec(html);
  var buddyStatus = (mBuddyStatus!==null) ? mBuddyStatus[1] : 'none';
...
/**
 * Parse the armory from HTML to an object
 * @param html
 * @returns {Object} parsed armory
 */
parser.parseArmory = function (html) {
  ...
  while ((mSellFor = reSellFor.exec(html)) !== null) {
    weaponsSellFor.push({
      ...
      price: helpers.quantityFromString(mSellFor[2]),
    });
  }
  // Repair
  ...
  [reCurrentWeapons, reCurrentTools].forEach(function (regex) {
    while ((mCurrentWeapons = regex.exec(html)) !== null) {
      ...
      var sell = {};
      var weaponId = -1;
      if (weaponsSellFor.length) {
        sell = weaponsSellFor.shift();
        weaponId = helpers.quantityFromString(sell.inputName);
      }
      var quantity = helpers.quantityFromString(mCurrentWeapons[2]);
      var strengthSplit = mCurrentWeapons[3].split('/');
      var strengthCurrent = helpers.quantityFromString(mCurrentWeapons[3]);
      var strengthMax = strengthCurrent;
      if(strengthSplit.length>1){
        strengthCurrent = helpers.quantityFromString(strengthSplit[0]);
        strengthMax = helpers.quantityFromString(strengthSplit[1]);
      }
      var currentWeapon = {
        name: mCurrentWeapons[1],
        weaponId: weaponId,
        quantity: quantity,
        strengthText: mCurrentWeapons[3],
        strengthCurrent: strengthCurrent,
        strengthMax: strengthMax,
        sell: sell,
      };
      if(repairWeapons[weaponId]!==undefined){
        currentWeapon.repair = repairWeapons[weaponId];
      }
      currentWeapons[category].push(currentWeapon);
    }
  });
...
parser.parsePersonnel = function (html) {
  ...
  while ((mPersonnel = rePersonnel.exec(html)) !== null) {
    personnel[mPersonnel[1]] = helpers.quantityFromString(mPersonnel[2]);
    personnel[mPersonnel[1]+' Text'] = mPersonnel[2];
  }
  return personnel;
};
...
parser.parseRecruit = function (html) {
  ...
  var reRecruitPref = /input\s*type\s*=\s*["']radio["']\s*name\s*=\s*["']\s*([^'"]+)["']\s*value\s*=\s*["']\s*([^'"]+)["']\s*([^>]*)>\s*([^<]+)/gmi;
  var mRecruitPref;
  var recruitPref = [];
  var recruitPrefName = "clickPref";
  var recruitPrefChecked = "";
  while ((mRecruitPref = reRecruitPref.exec(html)) !== null) {
    recruitPrefName = mRecruitPref[1];
    var checked = mRecruitPref[3].trim() == 'checked';
    if(checked)
      recruitPrefChecked = mRecruitPref[4];
    recruitPref.push({
      name: mRecruitPref[4],
  ...
  if(html.indexOf("There is no one left to click.") >= 0 ) {
    return {
      success: false,
      error: "There is no one left to click.",
      recruitPreference: recruitPrefChecked,
  ...
  var reUser = /recruit for <b><a href="stats\.php\?id=([0-9]+)">\s*([^<]+)\s*<\/a>/gmi;
  var mUser = reUser.exec(html);
  var user = mUser === null ? {
    username: "",
    userid: -1,
  } : {
    username: mUser[2],
    userid:   helpers.quantityFromString(mUser[1]),
  };
...
parser.parseBattleReport = function (html, location, userid) {
  // attack_id from location
  var attackId = -1;
  if(location!==undefined && location!==null && location.length){
    var reAttackId = /attack_id=([0-9]+)/;
    var mAttackId  = reAttackId.exec(location);
    attackId = (mAttackId!==null) ? helpers.quantityFromString(mAttackId[1]) : -1;
  }
...
parser.parseIntelFile = function (html) {
  ...
  // Username
  var reUsername = /Intelligence on ([^<]+)/gmi;
  var mUsername  = reUsername.exec(html);
  var username = mUsername === null ? "" : mUsername[1];
...
// Parse Intel Details
parser.parseIntelDetail = function (html, location, userid) {

  // report_id from location
  var reportId = -1;
  if(location!==undefined && location!==null && location.length){
    var reReportId = /report_id=([0-9]+)/;
    var mReportId  = reReportId.exec(location);
    reportId = (mReportId!==null) ? helpers.quantityFromString(mReportId[1]) : -1;
  }
  ...
  if( storyStart >= 0 )
  {
    ...
    story = helpers.stripHtml(html.substr(storyStart, storyEnd-storyStart)).trim();
    // Find out the username from the story
    var reUsername = /into ([^']+)'s/gmi;
    var mUsername = reUsername.exec(story);
    username = (mUsername!==null) ? mUsername[1] : "";
  }
  ...
  var spySuccess = mTreasury !== null;
  var success = spySuccess || story.length > 0;
  ...
  return {
    ...
    username: username,
    success: success,
    spySuccess: spySuccess,
    reportId: reportId,
    userid: userid !== undefined ? userid : -1,
  };
};

parser.parseInbox = function(inboxHtml) {

	var reTotal = /([0-9]+) messages total \| page ([0-9]+) of ([0-9]+)/gmi;

	// Inbox Info
	var inboxMatch = reTotal.exec(inboxHtml);
	var inboxInfo = {
		totalMessages: 0,
		currentPage: 1,
		totalPages: 1
	...
	if(inboxMatch!==null){
		inboxInfo.totalMessages = helpers.quantityFromString(inboxMatch[1]);
		inboxInfo.currentPage = helpers.quantityFromString(inboxMatch[2]);
		inboxInfo.totalPages = helpers.quantityFromString(inboxMatch[3]);
	}

	// Outbox Info
	var outboxMatch = reTotal.exec(inboxHtml);
	var outboxInfo = {
		totalMessages: 0,
		currentPage: 1,
		totalPages: 1
	...
	if(outboxMatch!==null){
		outboxInfo.totalMessages = helpers.quantityFromString(outboxMatch[1]);
		outboxInfo.currentPage = helpers.quantityFromString(outboxMatch[2]);
		outboxInfo.totalPages = helpers.quantityFromString(outboxMatch[3]);
	}

	// Inbox
	var reMessage = /<a href="stats\.php\?id=([0-9]+)">([^<]+)<\/b>\s*<\/td>\s*<td>([^<]+)<\/td>\s*<td align="right">([^<]+)<\/td>\s*<td align="left">([^<]+)<\/td>\s*<td[^>]*>\s*<form action="writemail\.php"[^>]*>\s*<input type="hidden" name="to" value="([0-9]+)">[\s\S]*?<input type="hidden" name="reply_to_message_id" value="([0-9]+)">[\s\S]*?<td style="[^"]*" bgcolor="[^"]*" colspan="4"><p style="[^"]*">([\s\S]*?)<\/p><\/td>/gmi;

	var inboxMessages = [];
	var messageInfoMatch, messageContentMatch;
	while((messageInfoMatch = reMessageInfo.exec(inboxHtml)) !== null)	{
		var message = {
			from: {
				userid: helpers.quantityFromString(messageInfoMatch[1]),
				username: messageInfoMatch[2],
			},
			subject: messageInfoMatch[3],
			when: messageInfoMatch[4] + " " + messageInfoMatch[5],
			messageId: helpers.quantityFromString(messageInfoMatch[6]),
		};
		if((messageContentMatch = reMessageContent.exec(inboxHtml)) !== null){
			message.content = messageContentMatch[1]; //.replace(/\n/g,'').replace(/<br \/>/g,'\n');
		}
	...
	return {
		...
		inbox: {
			...
			currentPage: inboxInfo.currentPage,
			totalPages: inboxInfo.totalPages,
			messages: inboxMessages
		},
		outbox: {
			totalMessages: outboxInfo.totalMessages,
			currentPage: outboxInfo.currentPage,
			totalPages: outboxInfo.totalPages,
			messages: [], // TODO
		}
	};
};
