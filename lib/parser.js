...
parser.parseBattleReport = function (html, location, userid) {
  ...
  var re = /<td class="report"[^>]*>([\s\S]*?)<\/td>/gmi;
  var m = re.exec(html);
  if (m === null) {
    return {
      success: false,
...
parser.parseInbox = function(inboxHtml) {
	...
	// Inbox
	var reMessage = /<a href="stats\.php\?id=([0-9]+)">([^<]+)<\/b>\s*<\/td>\s*<td>([^<]+)<\/td>\s*<td align="right">([^<]+)<\/td>\s*<td align="left">([^<]+)<\/td>\s*<td[^>]*>\s*<form action="writemail\.php"[^>]*>[\s\S]*?<input type="hidden" name="reply_to_message_id" value="([0-9]+)">[\s\S]*?<td style="[^"]*" bgcolor="[^"]*" colspan="4"><p style="[^"]*">([\s\S]*?)<\/p><\/td>/gmi;

	var inboxMessages = [];
	var messageMatch;
	while((messageMatch = reMessage.exec(inboxHtml)) !== null)	{
		var message = {
			from: {
				userid: helpers.quantityFromString(messageMatch[1]),
				username: messageMatch[2]
			},
			subject: messageMatch[3],
			when: messageMatch[4] + " " + messageMatch[5],
			messageId: helpers.quantityFromString(messageMatch[6]),
