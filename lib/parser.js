...
parser.parseBattlefield = function(html) {
  var reLoggedIn  = /<td align="center" valign="middle" style="padding: 0">\s*(.*)\s*(.*)\s*<\/td>\s*<td.*><a href="alliances\.php\?id=([^"]*)">[^<]*<\/a><\/td>\s*<td><a class="player" href="\/stats\.php\?id=([0-9]+)"\s*>([^<]+)<\/a><\/td>[^>]*>([0-9,]+)<\/td>[^>]*>\s*([A-Za-z0-9-_]+)\s*<\/td>[^>]*>([^G]+)Gold<\/td>[^>]*>([^<]+)<\/td>/gmi;
  var reLoggedOut = /<a href="alliances\.php\?id=([^"]*)">[^<]*<\/a><\/td>\s*<td><a class="player" href="\/stats\.php\?id=([0-9]+)"\s*>([^<]+)<\/a><\/td>[^>]*>([0-9,]+)<\/td>[^>]*>\s*([A-Za-z0-9-_]+)\s*<\/td>[^>]*>([^G]+)Gold<\/td>[^>]*>([^<]+)<\/td>/gmi;
  var players = [];
  var loggedIn = false;
  var m;
  [
    ...
    if(!players.length) { // do not check reLoggedOut if reLoggedIn worked
      while ((m = currentCase.re.exec(html)) !== null) {
        ...
        player.alliance     = m[startFrom];
        player.userid       = m[startFrom+1];
        player.username     = m[startFrom+2];
        player.armySizeText = m[startFrom+3];
        player.armySize     = helpers.quantityFromString(m[startFrom+3]);
        player.race         = m[startFrom+4];
        player.goldText     = m[startFrom+5].trim();
        player.gold         = helpers.quantityFromString(m[startFrom+5], -1);
        player.rankText     = m[startFrom+6];
        player.rank         = helpers.quantityFromString(m[startFrom+6], -1);
        players.push(player);
      }
    }
  });
...
parser.parseRecruit = function(html) {
  ...
  var imageHtml = eval("String( '" + mRecruitJavascript[1] + ")");
