...
parser.parseBattlefield = function(html) {
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    var player = {
      ...
      race: m[5],
      goldText: m[6],
      gold: helpers.quantityFromString(m[6], -1),
      rank: m[7]
    };
    players.push(player);
  }
...
parser.parseTraining = function(html) {
  ...
  //
  var reUpgrades = [
    /<th.*>([^<]+)<\/th>\s*<\/tr>\s*.*\s*<th.*>([^<]+)<\/th>\s*<th.*>([^<]+)<\/th>\s*.*\s*.*\s*<td>([^<]+)<\/td>\s*.*\s*.*type="submit"\s*value="([^"]+)".*\s*.*type="hidden"\s*name="([^"]+)"\s*value="([^"]+)/gmi,
    //gmi,
  ];
  var mUpgrades;
  var upgrades = [];
